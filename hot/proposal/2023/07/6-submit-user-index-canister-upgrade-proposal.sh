...
#   --summary "This proposal includes Step 2 (final step) for removing a legacy access_control field from the canister internal storage"  \
#   --url "${APP_URL}"  \
...
./quill sns \
  ...
  make-upgrade-canister-proposal \
  --title "Upgrade ${CANISTER_NAME} canister"  \
  --summary "This proposal refactors user_index internal data structure. It moves the known_principal map under configuration field and adds a new configuration value for sign-up control"  \
  --url "${APP_URL}"  \
  --target-canister-id "$(dfx canister id ${CANISTER_NAME} --network ic)" \
  --wasm-path "./.dfx/ic/canisters/${CANISTER_NAME}/${CANISTER_NAME}.wasm.gz" \
  --canister-upgrade-arg "(record {})" \
"54f9ba2b0e81a17f5261b277abd91816e041c5ca749ba88b3dbe05f66bb6124d" > "./sns/proposal/2023/07/6-submit-user-index-canister-upgrade-proposal.json"
./quill send "./sns/proposal/2023/07/6-submit-user-index-canister-upgrade-proposal.json" --yes
